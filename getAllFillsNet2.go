...
func main() {
	...
	for scFill.Scan() {
		...
		case "gap":
			...
			// this may need changing too

			newGap.Strand = words[4]
			if newGap.Strand == "-" {
				newGap.QueStart, newGap.QueEnd = newGap.QueEnd + 2, newGap.QueStart - 2
			}
		...
		case "fill":
			...
			if newFill.Strand == "-" {
				newFill.QueStart, newFill.QueEnd = newFill.QueEnd, newFill.QueStart
			}
